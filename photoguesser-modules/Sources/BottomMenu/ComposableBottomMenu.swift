...
extension BottomMenuState {
	fileprivate func converted(
		...
		.init(
			...
			footerButton: self.footerButton.converted(send: send, sendWithAnimation: sendWithAnimation)
		)
	}
}
...
#if DEBUG
import Styleguide
private struct BottomMenuPreviewReducer: ReducerProtocol {
	...
	struct BottomMenu_TCA_Previews: PreviewProvider {
		struct TestView: View {
			private let store = Store(
				initialState: nil,
				reducer: BottomMenuPreviewReducer()
			)
